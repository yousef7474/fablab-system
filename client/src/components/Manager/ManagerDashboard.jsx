...
const ManagerDashboard = () => {
  ...
  const [showEducationStatusModal, setShowEducationStatusModal] = useState(false);
  ...
  const handleCreateEmployee = async () => {
    ...
    // Validate section
    const finalSection = employeeForm.isCustomSection ? employeeForm.customSection : employeeForm.section;
    if (!finalSection) {
      toast.error(isRTL ? 'يرجى اختيار أو إدخال القسم' : 'Please select or enter a section');
      return;
    }
    ...
    try {
      await api.post('/admin/employees', {
        ...
        isCustomSection: employeeForm.isCustomSection
      });
      toast.success(isRTL ? 'تم إضافة الموظف بنجاح' : 'Employee added successfully');
      fetchEmployees();
      setShowEmployeeModal(false);
      setEmployeeForm({ name: '', email: '', section: '', customSection: '', isCustomSection: false });
    } catch (error) {
  ...
  const handleUpdateEmployee = async () => {
    ...
    setEmployeeLoading(true);
    try {
      await api.put(`/admin/employees/${selectedEmployee.employeeId}`, {
        name: employeeForm.name,
        email: employeeForm.email,
        section: finalSection,
        isCustomSection: employeeForm.isCustomSection
  ...
  const openEmployeeModal = (employee = null) => {
    if (employee) {
      ...
      // Check if section is custom (not in predefined list)
      const predefinedSections = Object.keys(SECTION_COLORS);
      const isCustom = employee.isCustomSection || !predefinedSections.includes(employee.section);
      setEmployeeForm({
        name: employee.name || '',
        email: employee.email || '',
        section: isCustom ? '' : (employee.section || ''),
        customSection: isCustom ? (employee.section || '') : '',
  ...
  const handleCreateWorkspace = async () => {
    ...
    try {
      ...
      setWorkspaceForm({
        tableNumber: '', projectName: '', numberOfUsers: 1, personName: '', personPhone: '', personEmail: '',
  ...
  // Fetch educations when education tab is active
  useEffect(() => {
  ...
  const handleDeleteEvaluation = async (id) => {
    if (!window.confirm(isRTL ? 'هل أنت متأكد من حذف هذا التقييم؟' : 'Delete this evaluation?')) return;
    try {
  ...
  }, [activeTab, managerData, fetchRatings, fetchEvaluations]);
  ...
  }, [activeTab, managerData, fetchGroupedTasks, fetchInterns]);
  ...
  const handleEducationStatusUpdate = async (educationId, status) => {
    try {
      ...
      setShowEducationStatusModal(false);
  ...
  const handleAddEducationRating = async () => {
    if (!selectedEducation) return;
    try {
      await api.post(`/education/${encodeURIComponent(selectedEducation.educationId)}/ratings`, educationRatingForm);
      toast.success(isRTL ? 'تم إضافة التقييم بنجاح' : 'Rating added successfully');
      setShowEducationRatingModal(false);
      setEducationRatingForm({ ratingDate: new Date().toISOString().split('T')[0], cleanlinessScore: 5, damageLevel: 'none', damageDescription: '', roomPhoto: '', comments: '' });
  ...
  const handleEducationRatingPhotoUpload = (e) => {
    const file = e.target.files[0];
    if (!file) return;
    ...
    const reader = new FileReader();
    reader.onload = (event) => setEducationRatingForm(prev => ({ ...prev, roomPhoto: event.target.result }));
    reader.readAsDataURL(file);
  };
      ...
      <aside className={`admin-sidebar ${sidebarOpen ? 'open' : 'collapsed'}`}>
        ...
        <nav className="sidebar-nav">
          ...
          <button
            ...
            onClick={() => { setActiveTab('education'); if (window.innerWidth <= 768) setSidebarOpen(false); }}
          >
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  ...
                  {filtered.map(task => (
                    <div key={task.taskId} className={`assignment-card priority-${task.priority} ${task.status === 'completed' ? 'completed-task' : ''} ${task.status === 'cancelled' ? 'cancelled-task' : ''}`}
                      ...
                      <div className="assignment-meta">
                        ...
                        ) : task.employeeCreator ? (
                          <div className="assignment-meta-item">
                            <span style={{ fontSize: '12px', color: '#3b82f6', fontWeight: 600 }}>{isRTL ? 'مهمة ذاتية من الموظف' : 'Self-created by employee'}</span>
                          </div>
                        ) : null}
                        {task.section && (
                          <div className="assignment-meta-item">
            ...
            >
              ...
              <div className="modern-modal-body">
                ...
                <div className="form-group modern-input">
                  ...
                  <select
                    value={employeeForm.isCustomSection ? 'other' : employeeForm.section}
                    onChange={(e) => {
                      if (e.target.value === 'other') {
                        setEmployeeForm({ ...employeeForm, section: '', isCustomSection: true });
                      } else {
                        setEmployeeForm({ ...employeeForm, section: e.target.value, customSection: '', isCustomSection: false });
                      }
                    }}
                    className="modern-input-field"
                  >
                    ...
                    <option value="other">{isRTL ? 'أخرى (إدخال يدوي)' : 'Other (Enter manually)'}</option>
                  </select>
                </div>
                {employeeForm.isCustomSection && (
                <div className="form-group">
                  <label>
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                      <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/>
                    ...
                    {isRTL ? 'اسم القسم المخصص' : 'Custom Section Name'} <span className="required">*</span>
                  </label>
                  <input
                    type="text"
                    value={employeeForm.customSection}
                    onChange={(e) => setEmployeeForm({ ...employeeForm, customSection: e.target.value })}
                  ...
                  <div className="form-group modern-input">
                    ...
                    <div className="input-with-icon">
                      ...
                      <input
                        ...
                        placeholder={isRTL ? 'اسم المشروع أو الغرض' : 'Project name or purpose'}
                        className="modern-input-field"
                      />
                    </div>
                  </div>
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
                ...
                {loadingEducations ? (
                  ...
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                    <button onClick={() => { setSelectedEducation(null); setEducationRatings([]); }} style={{ background: 'none', border: '1px solid var(--border-color)', borderRadius: '8px', padding: '8px 16px', cursor: 'pointer', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '6px', color: 'var(--text-primary)' }}>
                    ...
                    <div style={{ background: 'var(--card-bg)', borderRadius: '12px', padding: '24px', border: '1px solid var(--border-color)' }}>
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '12px' }}>
                        ...
                        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                          {selectedEducation.status === 'pending' && (
                            <>
                              <button onClick={() => { setEducationModalAction('approve'); setShowEducationStatusModal(true); }} style={{ padding: '8px 16px', borderRadius: '8px', background: '#22c55e', color: 'white', border: 'none', cursor: 'pointer', fontWeight: '600', fontSize: '13px' }}>{isRTL ? 'قبول' : 'Approve'}</button>
                              <button onClick={() => { setEducationModalAction('reject'); setShowEducationStatusModal(true); }} style={{ padding: '8px 16px', borderRadius: '8px', background: '#ef4444', color: 'white', border: 'none', cursor: 'pointer', fontWeight: '600', fontSize: '13px' }}>{isRTL ? 'رفض' : 'Reject'}</button>
                            </>
                          )}
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '12px', marginBottom: '20px' }}>
                        <div style={{ padding: '12px', background: 'var(--hover-bg)', borderRadius: '8px' }}>
                          <div style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>{isRTL ? 'المعلم' : 'Teacher'}</div>
                          <div style={{ fontWeight: '600', color: 'var(--text-primary)' }}>{selectedEducation.user?.firstName} {selectedEducation.user?.lastName}</div>
                        </div>
                        <div style={{ padding: '12px', background: 'var(--hover-bg)', borderRadius: '8px' }}>
                          <div style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>{isRTL ? 'الهاتف' : 'Phone'}</div>
                        ...
                        <div style={{ padding: '12px', background: '#f5f3ff', borderRadius: '8px' }}>
                          ...
                          <div style={{ fontWeight: '600', color: '#6d28d9' }}>{sectionLabels[selectedEducation.section] || selectedEducation.section}{selectedEducation.otherSectionDescription ? ` - ${selectedEducation.otherSectionDescription}` : ''}</div>
                        </div>
                        <div style={{ padding: '12px', background: '#f5f3ff', borderRadius: '8px' }}>
                        ...
                        <div style={{ padding: '12px', background: 'var(--hover-bg)', borderRadius: '8px' }}>
                          <div style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>{isRTL ? 'الوقت اليومي' : 'Daily Time'}</div>
                      ...
                      {selectedEducation.roomPhotoBefore && (
                        <div style={{ marginBottom: '20px' }}>
                          ...
                          <img src={selectedEducation.roomPhotoBefore} alt="Room Before" style={{ maxWidth: '300px', maxHeight: '200px', borderRadius: '8px', border: '2px solid var(--border-color)', objectFit: 'cover' }} />
                        </div>
                      )}
                      ...
                      <div style={{ marginTop: '24px' }}>
                        ...
                        {educationRatings.length === 0 ? (
                          <p style={{ color: 'var(--text-secondary)', textAlign: 'center', padding: '20px' }}>{isRTL ? 'لا توجد تقييمات بعد' : 'No ratings yet'}</p>
                        ) : (
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                            {educationRatings.map((rating) => (
                              <div key={rating.ratingId} style={{ padding: '16px', background: 'var(--hover-bg)', borderRadius: '12px', border: '1px solid var(--border-color)', position: 'relative' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                    <span style={{ fontWeight: '700', color: 'var(--text-primary)' }}>{rating.ratingDate}</span>
                                ...
                                {rating.damageDescription && <p style={{ margin: '4px 0', color: 'var(--text-secondary)', fontSize: '13px' }}>{rating.damageDescription}</p>}
                                {rating.comments && <p style={{ margin: '4px 0', color: 'var(--text-primary)', fontSize: '13px' }}>{rating.comments}</p>}
                                ...
                                <div style={{ fontSize: '11px', color: 'var(--text-secondary)', marginTop: '4px' }}>{isRTL ? 'بواسطة' : 'By'}: {rating.ratedBy?.fullName || 'N/A'}</div>
                              </div>
                            ))}
                          </div>
                        )}
                  ...
                  <div>
                    <div className="table-container">
                      <table className="data-table">
                        ...
                        <tbody>
                          {educations.length === 0 ? (
                            <tr><td colSpan="7" style={{ textAlign: 'center', padding: '40px', color: 'var(--text-secondary)' }}>{isRTL ? 'لا توجد طلبات تعليم' : 'No education requests'}</td></tr>
                          ) : educations.map((edu) => (
                            <tr key={edu.educationId}>
                              <td style={{ fontWeight: '600' }}>{edu.educationId}</td>
                              <td>{edu.user?.firstName} {edu.user?.lastName}</td>
                              <td>{sectionLabels[edu.section] || edu.section}</td>
                    ...
                    {educationPagination.pages > 1 && (
                      <div style={{ display: 'flex', justifyContent: 'center', gap: '4px', marginTop: '16px' }}>
                        {Array.from({ length: educationPagination.pages }, (_, i) => i + 1).map(pageNum => (
                          <button key={pageNum} onClick={() => fetchEducations(pageNum)} style={{ padding: '6px 12px', borderRadius: '6px', border: '1px solid var(--border-color)', background: pageNum === educationPagination.page ? '#6d28d9' : 'var(--card-bg)', color: pageNum === educationPagination.page ? 'white' : 'var(--text-primary)', cursor: 'pointer', fontSize: '13px' }}>{pageNum}</button>
                        ))}
                      </div>
                    )}
                  </div>
                ...
                {showEducationStatusModal && selectedEducation && (
                  <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
                    <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} style={{ background: 'var(--card-bg)', borderRadius: '16px', padding: '24px', maxWidth: '400px', width: '90%' }}>
                      ...
                      <textarea
                        ...
                        style={{ width: '100%', padding: '12px', borderRadius: '8px', border: '1px solid var(--border-color)', minHeight: '80px', resize: 'vertical', marginBottom: '16px', fontFamily: 'inherit', background: 'var(--card-bg)', color: 'var(--text-primary)' }}
                      />
                      <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
                        <button onClick={() => { setShowEducationStatusModal(false); setEducationAdminNotes(''); }} style={{ padding: '8px 16px', borderRadius: '8px', border: '1px solid var(--border-color)', background: 'var(--card-bg)', cursor: 'pointer', color: 'var(--text-primary)' }}>{isRTL ? 'إلغاء' : 'Cancel'}</button>
                ...
                {/* Education Rating Modal */}
                {showEducationRatingModal && (
                  <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
                    <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} style={{ background: 'var(--card-bg)', borderRadius: '16px', padding: '24px', maxWidth: '500px', width: '90%', maxHeight: '80vh', overflowY: 'auto' }}>
                      ...
                      <div style={{ marginBottom: '16px' }}>
                        ...
                        <input type="date" value={educationRatingForm.ratingDate} onChange={(e) => setEducationRatingForm({ ...educationRatingForm, ratingDate: e.target.value })} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1px solid var(--border-color)', background: 'var(--card-bg)', color: 'var(--text-primary)' }} />
                      </div>

                      <div style={{ marginBottom: '16px' }}>
                        <label style={{ display: 'block', marginBottom: '4px', fontWeight: '600', fontSize: '14px', color: 'var(--text-primary)' }}>{isRTL ? 'النظافة' : 'Cleanliness'} ({educationRatingForm.cleanlinessScore}/5)</label>
                        <div style={{ display: 'flex', gap: '8px' }}>
                          {[1, 2, 3, 4, 5].map(score => (
                            <button key={score} onClick={() => setEducationRatingForm({ ...educationRatingForm, cleanlinessScore: score })} style={{ fontSize: '28px', background: 'none', border: 'none', cursor: 'pointer', color: score <= educationRatingForm.cleanlinessScore ? '#f59e0b' : 'var(--border-color)' }}>★</button>
                          ))}
                        </div>
                      </div>
                      ...
                      <div style={{ marginBottom: '16px' }}>
                        ...
                        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                          {['none', 'minor', 'moderate', 'severe'].map(level => (
                            <button key={level} onClick={() => setEducationRatingForm({ ...educationRatingForm, damageLevel: level })} style={{ padding: '8px 16px', borderRadius: '8px', border: `2px solid ${educationRatingForm.damageLevel === level ? getDamageLevelColor(level) : 'var(--border-color)'}`, background: educationRatingForm.damageLevel === level ? `${getDamageLevelColor(level)}22` : 'var(--card-bg)', color: getDamageLevelColor(level), cursor: 'pointer', fontWeight: '600', fontSize: '13px' }}>
                      ...
                      {educationRatingForm.damageLevel !== 'none' && (
                        <div style={{ marginBottom: '16px' }}>
                          ...
                          <textarea value={educationRatingForm.damageDescription} onChange={(e) => setEducationRatingForm({ ...educationRatingForm, damageDescription: e.target.value })} placeholder={isRTL ? 'صف الضرر' : 'Describe the damage'} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1px solid var(--border-color)', minHeight: '60px', resize: 'vertical', fontFamily: 'inherit', background: 'var(--card-bg)', color: 'var(--text-primary)' }} />
                        </div>
                      )}
                      ...
                      <div style={{ marginBottom: '16px' }}>
                        ...
                        <input type="file" accept="image/*" onChange={handleEducationRatingPhotoUpload} style={{ fontSize: '13px' }} />
                        {educationRatingForm.roomPhoto && <img src={educationRatingForm.roomPhoto} alt="Rating" style={{ maxWidth: '150px', maxHeight: '100px', borderRadius: '6px', marginTop: '8px', objectFit: 'cover' }} />}
                      </div>
                      ...
                      <div style={{ marginBottom: '16px' }}>
                        ...
                        <textarea value={educationRatingForm.comments} onChange={(e) => setEducationRatingForm({ ...educationRatingForm, comments: e.target.value })} placeholder={isRTL ? 'تعليقات إضافية' : 'Additional comments'} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1px solid var(--border-color)', minHeight: '60px', resize: 'vertical', fontFamily: 'inherit', background: 'var(--card-bg)', color: 'var(--text-primary)' }} />
                      </div>
