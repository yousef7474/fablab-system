      ...
      if (/^\d{6,15}$/.test(raw)) {
        try {
          ...
          try {
            ...
            // Neither student nor volunteer — try FabLab staff as a last resort
            try {
              const sRes = await api.post('/fablab-staff/attendance/scan', { code: raw });
              const st = sRes.data.staff || {};
              const action = sRes.data.action;
              const rec = sRes.data.record || {};
              const fmt = (iso) => {
                if (!iso) return '';
                const d = new Date(iso);
                const pad = (n) => String(n).padStart(2, '0');
              ...
              setWelcome({ name: st.name || raw, workshop, color, phone: st.phone || '' });
              if (welcomeTimerRef.current) clearTimeout(welcomeTimerRef.current);
              welcomeTimerRef.current = setTimeout(() => {
                setWelcome(null);
                cooldownRef.current = false;
              ...
              // Neither student nor volunteer nor staff — try University
              // Training intern before giving up.
              try {
                const iRes = await api.post('/interns/attendance/scan', { code: raw });
                const it = iRes.data.intern || {};
                const action = iRes.data.action;
                const rec = iRes.data.record || {};
                const fmt = (iso) => {
                  if (!iso) return '';
                  const d = new Date(iso);
                  const pad = (n) => String(n).padStart(2, '0');
                ...
                setWelcome({ name: it.name || raw, workshop, color, phone: it.phone || '' });
                if (welcomeTimerRef.current) clearTimeout(welcomeTimerRef.current);
                welcomeTimerRef.current = setTimeout(() => {
                  setWelcome(null);
                  cooldownRef.current = false;
                ...
                setWelcome({
                  name: raw,
                  workshop: internErr?.response?.data?.message
                    || staffErr?.response?.data?.message
                    || volErr?.response?.data?.message
